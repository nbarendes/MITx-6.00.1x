...
WORDLIST_FILENAME = "C:\\Users\\Nicolas\\Desktop\\Ps4\\words.txt"
...
def isValidWord(word, hand, wordList):
    ...
    l_check = set(list(word))<=set(ans.keys())

    for letter in word:
        if letter in ans.keys():
            ans[letter]-=1

    value_check = all(i>=0 for i in ans.values())

    if (w_check and l_check and value_check):
        return True
    else:
        return False
